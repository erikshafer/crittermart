...
// The Cart aggregate — the domain WRITE model (ADR 020). `sealed` and immutable: every Apply
// returns a NEW Cart via `with`, never mutates in place. It is the FetchForWriting / StartStream
...
//
// It lives in the `Shopping/` **verb feature folder** (namespace `CritterMart.Orders.Shopping`), not a
// `Cart/` folder — that is the ADR 021 convention: a slice's folder is named for the activity, so the
// aggregate keeps its canonical domain-noun name `Cart` with no collision (a verb namespace never clashes
// with a noun type). The Order slice follows the same convention with an `Ordering/` folder around `Order`.
//
// Self-aggregating inline snapshot (ADR 008, refined by ADR 020): the static Create/Apply methods ARE the
// projection, registered via `Projections.Snapshot<Cart>(SnapshotLifecycle.Inline)`, so the aggregate
...
// stream key (AddToCart starts the stream as `StartStream<Cart>(cartId, new CartCreated(cartId, …))`).
