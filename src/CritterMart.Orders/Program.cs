...
builder.Services.AddMarten(opts =>
    {
        ...
        // The Cart aggregate — the domain WRITE model (ADR 020), a self-aggregating immutable record
        // materialized as an inline snapshot (ADR 008). It is the FetchForWriting/StartStream target and
        // is never served over HTTP. The open-cart invariant index (below) lives on it.
        ...
        // a partial-unique index on Cart.CustomerId, scoped to open carts, enforces "one open cart per
        // customer" at the DB. The write paths resolve the customer's open cart against this index; a
