...
The `customer-registry` capability is the Identity bounded context's customer registry **and** the system's authentication issuer — CritterMart's one deliberately **non-event-sourced** bounded context (EF Core + Npgsql on the shared PostgreSQL, schema-per-service), where the `Customer` row *is* the read model and `CustomerRegistered { customerId, email, displayName }` is an outbox **notification** rather than a domain-event stream. `RegisterCustomer` (`POST /customers`, a server-minted UUID or a caller-supplied explicit id for seed/admin provisioning) writes the row and enrolls `CustomerRegistered` in the EF-Core **transactional outbox in the same transaction**, so the event publishes only after the commit succeeds; normalized-email (`Trim().ToLowerInvariant()`) uniqueness is guarded twice — a pre-handle application guard returning `409 CustomerAlreadyRegistered` and a database unique index on the normalized column. `GET /customers/{id}` is the **Open-Host Service** read the storefront resolves against, and `CustomerRegistered` is the **Published Language** a cross-BC consumer subscribes to — Orders folds it into a consumer-local `LocalCustomerView` (slice 5.4) rather than ever calling back into Identity, honoring ADR 001's no-synchronous-service-to-service-HTTP rule. The capability also hosts the `EmailChange` **saga** — CritterMart's second convention `Wolverine.Saga`, backed by **EF-Core saga storage** (proving the saga store is swappable, Marten ↔ EF Core) — which opens on `RequestEmailChange`, applies the pending email on a timely `ConfirmEmailChange` (rejecting a `409 EmailChangeConflict` without completing when the address was claimed mid-window), and drops the change on an `EmailChangeTimeout`, with the confirm deadline scheduled exactly once and never rescheduled because Wolverine provides no scheduled-message cancellation. Per **ADR 023**, Identity is additionally the system's **sole auth issuer**: an ASP.NET Core Identity user store behind `POST /register` (`RegisterWithCredentials`, which creates the Identity user and the registry `Customer` row **all-or-nothing in one transaction**) and `POST /login`, which mints a self-validated **JWT** whose `sub` claim is the customer id, signed with Identity's asymmetric **private** (RSA) key and never persisted; resource servers — Orders — verify every presented bearer token **fully offline** against Identity's **config-distributed public key**, with zero HTTP into Identity per request or at startup, so auth adds no cross-BC runtime edge (the context map draws none). The `sub` claim is the **sole** customer trust boundary — the transitional dev-only `X-Customer-Id` fallback is fully retired (hard cutover; customer-keyed endpoints are blanket-`[Authorize]`'d, unauthenticated → `401`) — and logout is **client-side token discard**, since the stateless JWT carries no server session or denylist this increment (server-side revocation is a deferred future increment, ADR 023 Q15). This is the Identity BC's **sole capability**, spanning Workshop 002 slices 5.1–5.11: register/resolve (5.1/5.2), the Published-Language graduation + Orders consumer (5.3/5.4), the email-change saga (5.5–5.7), real authentication (5.8–5.11), and the `X-Customer-Id` hard cutover.
## Requirements
