...
- **The `Cart` type ↔ `…Cart` namespace collision (CS0118 / CS0234).** A type named `Cart` in namespace `CritterMart.Orders.Cart` is ambiguous in cross-namespace files — it bit the four `Features/` handlers and the test project, resolving `Cart` to the namespace. Worked around with a local `using CartAggregate = …Cart.Cart;` alias (the frozen prompt's "fully-qualify, don't rename" call). It is the session's clearest piece of surfaced debt: the clean fix is pluralizing `Cart/` → `Carts/` (MMO sidesteps it with `Claims/`), and it **recurs for `Order`** — decide before the Order pilot.
...
- **Surface a recurring structural smell as a gated decision, don't paper it silently.** The namespace collision is aliased for now but flagged as a pre-Order-pilot decision, so the workaround doesn't quietly become the convention.

## Outstanding / next-session inputs

- **Order and Stock pilots** reuse this template: `OrderStatusView` → `Order` (also the PMvH state) + an `OrderStatusView` read model; `StockLevelView` → `StockLevel` + a read model. Each its own PR.
- **Namespace pluralization decision (recommended before the Order pilot).** `Cart/` → `Carts/` (and `Order/` → ?) eliminates the type↔namespace collision and lets the `CartAggregate` aliases be deleted. It is a focused `refactor:` of its own; it was fenced out of this pilot to keep the read/write split diff clean.
