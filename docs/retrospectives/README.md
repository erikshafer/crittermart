...
Kinds populated for round one: `rules/` (one retro), `workshops/` (one retro), `docs/` (one retro — this folder-READMEs session). Additional kinds (`narratives/`, `skills/`, `decisions/`, `implementations/`) appear as their first session of that kind lands.
