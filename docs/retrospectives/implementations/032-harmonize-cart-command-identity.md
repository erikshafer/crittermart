...
- **Live Aspire boot — offered, not done this session.** The change is covered by real-Testcontainers Alba integration tests (real `[FromHeader]` binding + route table) + the frontend hook tests, and the header path mirrors the already-live read. A full Aspire boot (SPA hitting the new cart routes cross-origin in a browser) would be the belt-and-suspenders confirmation per the live-verify norm — offered to the owner; reasonable to run before merge or fold into the next live session.
