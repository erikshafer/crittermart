...
- [`011-customer-redeems-coupon.md`](011-customer-redeems-coupon.md) — the Customer's flash-coupon journey (v1.0, Orders BC — the Promotions lane), covering slices 6.1 (Define a coupon — configuration-as-events, seed-realized), 6.3 (Redeem at checkout under a global cap — CritterMart's **first Dynamic Consistency Boundary**, including the cap-breach `CouponExhausted` refusal and the `DcbConcurrencyException` race resolving to exactly one surviving order), and 6.4 (Release a redemption on cancellation — the slot returns to the pool). Wraps Narrative 004's Moment 2 (Place Order): `couponCode` rides `PlaceOrder` optionally, leaving the no-coupon checkout unchanged. The advisory cart-review preview (slice 6.2) is noted as forthcoming. Sibling of the `2026-07-16-slices-6-1-6-3-6-4-coupon-dcb` OpenSpec change ([ADR 024](../decisions/024-dcb-coupon-redemption-in-orders.md)).
