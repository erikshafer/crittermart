...
- [`004-customer-purchase.md`](004-customer-purchase.md) — the Customer's purchasing journey (v1.6, Orders BC), covering slices 3.1 (Add item to cart), 3.2/3.3 (cart edits — remove item, change quantity), 4.1 (Place order), 4.2/4.5 (stock reservation + cancel on stock failure), 4.3/4.4 (payment authorization + confirmation), 4.6 (cancel on payment decline with cross-BC stock release), and 4.7 (cancel on payment timeout — the order lifecycle's completing slice) across Moments 1–6 plus the journey-ordered insert Moment 1A; only abandonment (3.4) is forward-looked.
