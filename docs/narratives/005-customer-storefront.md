...
This narrative is **modeled, not yet built** — it is the screen lens of slices that are now in the model (slice 3.5 net-new; 1.2 / 3.1–3.3 / 4.1 already shipped on the backend). The next session is the **first frontend implementation slice**: the OpenSpec proposal and code for **slice 3.5 (View my open cart)** — the cleanest first slice, since it adds no new event and unblocks the cart-review screen. That session also establishes **`docs/skills/frontend/SKILL.md`** — deferred until now by design, so it documents the conventions (version pins, the Zod boundary, the optimistic-UI + rollback pattern) as the *code* establishes them rather than as guesses (ADR 015). As the browse / cart / checkout / track screens are built, this narrative grows — each implemented screen bumping its version and appending to the history below. Nothing in this journey is half-finished silently: every screen named here maps to a modeled slice and a wireframe; the build order is the audit's (Gap #1 first).
