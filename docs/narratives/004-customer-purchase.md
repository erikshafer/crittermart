...
status: draft
version: v1.3
slices: [3.1, 4.1, 4.2, 4.5, 4.3, 4.4]
references:
...
- **Slice 4.3 — Authorize payment (stubbed)**, and **slice 4.4 — confirm when both gates close.** Authored in this version. Moment 4 below covers the second gate and the journey's payoff: with stock reserved, Orders authorizes payment against a stubbed provider and — on approval — *confirms* the order, its terminal success state.

Forthcoming (not authored here): editing the cart — remove (3.2), change quantity (3.3); the *other* cancellations of order processing — payment decline (4.6) and payment timeout (4.7); and **cart abandonment** (3.4) if the cart is left inactive. Each extends this narrative when its slice is authored.
...
- **If payment is declined**, Orders records a `PaymentAuthFailed` on the stream — the refusal, captured as a first-class fact — and the order does **not** confirm. (Turning a declined order terminal, and handing the reserved stock back to Inventory, is its own step — slice 4.6 — authored later. Until then a declined order carries the failure on its record while its visible status stays `stock_reserved`.)
...
- **No cancellation of a *declined* order yet.** A reserved order now authorizes payment and, on approval, confirms (Moment 4). But on a *decline*, the order records `PaymentAuthFailed` and stops there — it is not yet turned terminal, and its reserved stock is not yet handed back. Cancelling a declined order, with the cross-BC stock release that needs (Inventory slice 2.3), is slice **4.6**, authored later. Cancellation on **stock** failure (4.5, Moment 3) is the only order cancellation that exists so far.
- **No payment timeout yet.** There is no `OrderPaymentTimeout` ticking; an order that never hears back from the (always-approving stub) provider has no deadline that cancels it. That scheduling — and the `OrdersAwaitingPayment*` projection that drives it — arrives with slice 4.7.
...
- **The remaining cancellations of order processing** (slices 4.6–4.7) — picking up where Moment 4 leaves a *declined* order: cancelled on payment decline (4.6, with the cross-BC stock release) or on payment timeout (4.7). (Stock reservation + stock-failure cancel are Moment 3; payment authorization + confirmation are Moment 4.)
