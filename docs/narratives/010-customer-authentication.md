...
| Version | Date       | Notes |
| ------- | ---------- | ----- |
| v1.1    | 2026-07-14 | Hard cutover lands (OpenSpec change `retire-x-customer-id-fallback`): Moment 3's "dev-only fallback lingers, tracked for removal" parenthetical is closed — the `X-Customer-Id` header no longer resolves any identity, the six customer-keyed Orders endpoints are blanket-`[Authorize]`'d, and an unauthenticated request is `401` (the pre-auth "missing header → 400" died with the header). The seeder, test suites, and `demo-traffic.ps1` all authenticate as real shoppers now. |
