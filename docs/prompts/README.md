...
Kinds populated for round one: `rules/` (one prompt), `workshops/` (one prompt), `docs/` (one prompt — this folder-READMEs session). Additional kinds (`narratives/`, `skills/`, `decisions/`, `implementations/`) appear as their first session of that kind lands.
