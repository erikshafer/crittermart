...
Kinds populated for round one: `rules/` (one prompt), `workshops/` (one prompt), `docs/` (multiple prompts — folder-READMEs, README overhaul, housekeeping sweep), `research/` (two prompts — ecommerce engineering lessons survey, frontend stack landscape), `narratives/` (one prompt — Seller's catalog-management journey for slice 1.1). Additional kinds (`skills/`, `decisions/`, `implementations/`) appear as their first session of that kind lands.
