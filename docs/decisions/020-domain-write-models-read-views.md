...
- **A type ↔ namespace collision surfaced.** A `Cart` type in namespace `CritterMart.Orders.Cart` is CS0118-ambiguous in cross-namespace files (the `Features/` handlers, the test project), worked around with a local `using CartAggregate = …Cart.Cart;` alias. The clean fix — pluralizing the folder/namespace (`Cart/` → `Carts/`, as MMO's `Claims/` does) — is deferred; it recurs for `Order` and should be decided before the Order pilot (retro 002).
