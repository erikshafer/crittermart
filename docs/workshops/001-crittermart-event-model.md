...
7. Promotions BC with DCB-protected coupon redemption (per context map § Long road). **→ RESOLVED (v1.13, 2026-07-15): chosen and modeled.** [ADR 024](../decisions/024-dcb-coupon-redemption-in-orders.md) chose the direction — a global per-coupon redemption cap enforced by Marten DCB **inside the Orders store**, Promotions contributing coupon *definitions* only (no fifth service this increment) — and [Workshop 003](003-promotions-event-model.md) models it: slices 6.1–6.4 add coupon definition (configuration-as-events), an advisory cart-review validation query, checkout redemption (tagged `CouponRedeemed` under the DCB cap check, composing with slice 4.1), and release-on-cancel (composing with slices 4.5–4.7) to the Orders lane. This workshop's slices 4.1 and 4.5–4.7 are behaviorally unchanged when no coupon is present; the coupon-carrying variants live in Workshop 003. Not yet implemented — the per-slice loop follows.
...
| v1.13   | 2026-07-15 | **Long-road parking-lot item 7 resolved by pointer** (Workshop 003 session, no model change here). ADR 024 chose Promotions via DCB (global per-coupon redemption cap enforced inside the Orders store; Promotions = coupon *definitions* only, standalone service deferred); the owner chose a **standalone Workshop 003** over amending this workshop (the Workshop 002 precedent: new concept = new workshop), so the event model — slices 6.1–6.4, the tagged `CouponRedeemed`/`CouponRedemptionReleased` pair, the `CouponUsage` DCB boundary — lives in [Workshop 003](003-promotions-event-model.md). This workshop's § 8 item 7 gains the resolved marker; slices 4.1 and 4.5–4.7 remain behaviorally unchanged when no coupon is present (the coupon-carrying variants are modeled as compositions in Workshop 003, not edits here). Slice table (§5) untouched. |
