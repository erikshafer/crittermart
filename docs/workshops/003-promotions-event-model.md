...
status: Modeled and IMPLEMENTED (slices 6.1/6.2/6.3/6.4/6.5/6.6) — the workshop is fully built. The invariant-bearing DCB core shipped via implementations/039; the advisory cart-review query + W2/W3 storefront UI (slice 6.2) shipped via implementations/040; the second DCB — one-redemption-per-customer, the composite (coupon × customer) boundary (slice 6.5) — shipped via implementations/041; the per-customer advisory preview + tailored refusal copy (slice 6.6) shipped via implementations/042 (OpenSpec change `slice-6-6-per-customer-preview-and-copy`).
version: v1.5
date: 2026-07-15
...
6. **One-redemption-per-customer** — ✅ **IMPLEMENTED (v1.3, slice 6.5, implementations/041).** The composite `(coupon × customer)` tag ADR 024 §38 named as the more dynamic boundary — CritterMart's second DCB and first composite-tag boundary, an opt-in `oneRedemptionPerCustomer` policy on the definition. Its two parked follow-ons — the per-customer **advisory preview** and **tailored refusal copy** — are ✅ **IMPLEMENTED as slice 6.6 (v1.5, implementations/042).** A `perCustomerLimit > 1` generalization is a definition-payload widening if ever wanted (the modeled invariant is exactly "once").
    - **The two questions slice 6.6 left to its implementation session are answered: both DECLINED** (implementations/042, `design.md` decision 6), and for the same structural reason rather than on taste. (a) A **badge advertising the `oneRedemptionPerCustomer` policy to anonymous shoppers** would require the validate response to carry the policy flag — but the spec pins the anonymous answer *identical to the behavior that shipped in slice 6.2*, so a badge visible to a signed-out shopper is precisely that pinned response gaining a field. It would also leak a definition detail to unauthenticated callers for the first time, a real (if small) widening of a public contract that deserves its own decision rather than riding a UX-polish slice. (b) A **sign-in nudge** depends on (a): firing it for *every* coupon is noise, and firing it only for per-customer coupons needs the same forbidden flag. It also inverts the slice's premise — 6.6 exists to move bad news *earlier*, not to add an interstitial ask before a shopper can see a discount. Both remain available as a later slice once there is a reason to widen the anonymous contract deliberately.
...
| v1.5    | 2026-07-20 | **Slice 6.6 IMPLEMENTED** (implementations/042, OpenSpec change `slice-6-6-per-customer-preview-and-copy` — `coupon-promotion` 2 MODIFIED + 1 ADDED). The model landed **as drawn**: both redemption events gained the optional defaulted `customerId` member; `CustomerCouponUsageView` shipped as an inline multi-stream projection keyed `"{couponId}\|{customerId}"`; `GET /coupons/{code}/validate` became optionally authenticated (no `[Authorize]`, never `401`) with the fourth status `already_redeemed` gated on *both* the definition's `oneRedemptionPerCustomer` **and** a non-empty caller `sub`, ordered `invalid` → `already_redeemed` → `exhausted` → `valid`; and the `409` detail was reworded with its status code and `CouponAlreadyRedeemedByCustomer` title token untouched. **No amendment to the model was needed** — the §4 tags-are-not-groupable finding held exactly as recorded, and the forward-only limitation was implemented as its own test rather than patched with a backfill. Two implementation notes worth the record: (1) `Auth/CustomerIdentity.CustomerId()` is deliberately **not** used by this endpoint — it *throws* on an absent `sub`, correct behind `[Authorize]` and wrong for an optionally-authenticated route where anonymous is the normal case, so the claim is read directly; and (2) the frontend needed *less* than the proposal's Impact assumed — the bearer token already rode the validate call via `authHeaders` (Convention 4), so the only load-bearing client change was widening the closed `CouponStatusSchema` enum, without which the new status would fail zod parsing and surface as a fetch error rather than copy. Unattributed pre-6.6 events fold into a `"{couponId}\|"` bucket no query ever constructs (an authenticated caller always has a non-empty `sub`) — filtering them out was considered and rejected as a second place for the forward-only rule to drift. §8 item 6's two named UI questions are answered — **both declined**, see item 6. Sibling amendment: Narrative 011 → v1.4. |
