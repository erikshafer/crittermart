...
- `client/` — the storefront SPA. Entry points: `src/config.ts` (Aspire-injected service URLs, Zod-validated), `src/api/client.ts` (shared fetch — `fetchParsed` read + `postCommand`/`deleteCommand` commands, header seam + boundary parse; `postCommand`'s `schema` is optional so a 204-No-Content command parses no body), `src/identity/useCurrentCustomer.tsx` (the ADR 009 seam), `src/router.tsx` (code-based router; `/` → W1 browse, `/cart` → W2), `src/catalog/` (the W1 browse feature folder — `catalogSchema.ts` / `catalogQueries.ts` / `BrowsePage.tsx`), `src/cart/` (the W2 + cart-command folder — `cartSchema.ts` / `cartQueries.ts` / `CartPage.tsx` (the editable cart, per-row `CartRow`) / `CartBadge.tsx` / `cartMutations.ts`, the optimistic-UI layer — `useAddToCart` + the W2 edits `useChangeCartItemQuantity` / `useRemoveCartItem`, each a thin hook over a pure merge sibling), `client/README.md` (run + layout). The Aspire wiring is in `src/CritterMart.AppHost/Program.cs` (`AddViteApp` + CORS injection).
