...
- [`updating-critter-stack-dependencies/SKILL.md`](updating-critter-stack-dependencies/SKILL.md) — the **operational runbook** for updating CritterMart's JasperFx / Critter Stack dependencies (Wolverine, CritterWatch, Alba; Marten / JasperFx / Weasel transitive). Encodes the policy: bump every package in a family to the latest NuGet stable in **one clean sweep** via `Directory.Packages.props`, `restore --force` + build + test to validate (incremental fallback only on a failed sweep), and **summarize the release notes** (not just the version delta) — including the Wolverine `V`-tag / `curl` public-API path and the fact that CritterWatch is closed-source with no public changelog. Carries the standing MessagePack-CVE-suppression re-check and the Dependabot-supersedes note. The first `ops`-cluster skill; this is a procedure + project-fact skill, not a code-convention one, so it defers all Wolverine/Marten *API* mechanics to the upstream library.
- [`frontend/SKILL.md`](frontend/SKILL.md) — CritterMart's storefront SPA conventions: the locked version pins, Zod at every one of the **three** service wire boundaries, optimistic-UI + rollback against the read model, the `useCurrentCustomer` identity seam carried as the `X-Customer-Id` header, and the no-BFF / CORS-in-dev-and-prod / no-proxy posture. The first non-`core` cluster (`frontend`). Authored as the **v1 seed** in the first frontend-touching slice (3.5 "View my open cart"), before `client/` exists — it records the conventions the frontend ADRs (015/016/018/009) already lock plus the one integration slice 3.5 establishes (the `GET /carts/mine` open-cart read), and is fleshed out as components land. Defers React/Vite/Tailwind/Zod/TanStack library mechanics to the installed per-library skills (`tanstack-query-best-practices`, `zod`, `shadcn`, `tailwind`, `react-hook-form`, …).
