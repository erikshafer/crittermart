...
| **Orders** | Cart, Order (process manager), payment timeout | Event-sourced (Marten) | In progress — **Order lifecycle complete** (4.1 placement, 4.2 / 4.5 cross-BC stock reservation + cancel-on-stock-failure, 4.3 / 4.4 stubbed payment authorization + confirmation, 4.6 cancel-on-payment-decline, 4.7 cancel-on-payment-timeout with the `OrdersAwaitingPayment` Bruun todo-list); Cart has 3.1 add-to-cart and 3.2 / 3.3 cart edits (remove item, change quantity — SKU-keyed lines); only abandonment (3.4) remains |
...
> **Status note.** CritterMart is in the **per-slice implementation phase** for round one. The design artifact suite (vision, context map, workshop, ADRs, rules, folder READMEs, skills, prompts, retrospectives) is complete, and `src/` is populated: three Wolverine services (Catalog, Inventory, Orders), the `CritterMart.AppHost` Aspire orchestrator, `CritterMart.ServiceDefaults`, and `CritterMart.Contracts` (the cross-BC published language). Slices ship per the per-slice loop — Catalog (1.1–1.3) and Inventory (2.1–2.3) are in, and the Orders BC's **Order lifecycle is complete** (3.1 add-to-cart, 4.1 place-order, 4.2/4.5 cross-BC stock reservation + cancel-on-stock-failure — the project's first live RabbitMQ traffic — 4.3/4.4 stubbed payment authorization + confirmation, 4.6 cancel-on-payment-decline with cross-BC stock release, and 4.7 cancel-on-payment-timeout — the project's first temporal automation; every placed order now reaches `confirmed` or `cancelled`), with the cart edits in too (3.2 remove-item, 3.3 change-quantity — SKU-keyed cart lines). Only cart abandonment (3.4) remains. The instructions below reflect the working local-development workflow.
