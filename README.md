...
| **Orders** | Cart, Order (process manager), payment timeout | Event-sourced (Marten) | In progress — **Order lifecycle complete** (4.1 placement, 4.2 / 4.5 cross-BC stock reservation + cancel-on-stock-failure, 4.3 / 4.4 stubbed payment authorization + confirmation, 4.6 cancel-on-payment-decline, 4.7 cancel-on-payment-timeout with the `OrdersAwaitingPayment` Bruun todo-list); Cart has 3.1 add-to-cart; cart edits (3.2 / 3.3) and abandonment (3.4) forthcoming |
