...
public class ChangeCartItemQuantityTests
{
    ...
    // Hard cutover (ADR 023): a quantity-change with no Bearer token is rejected with 401 by [Authorize]
    // before the quantity guard or open-cart resolution runs — unauthenticated, mirroring the cart read
    // and the other cart commands (the pre-cutover "missing header → 400" died with the header).
    [Fact]
    public async Task changing_quantity_without_a_token_returns_401()
    {
        await ResetOrdersAsync();
